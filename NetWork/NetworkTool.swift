...
class NetworkTool: Alamofire.Manager {
    ...
    /// 请求用户详情 就是用户所发表的文章
	func getUserDetail(userID: String, finished: (author: Author?, error: NSError?) -> Void) {

		let dic = ["userID": userID]
		request(.GET, "http://m.htxq.net/servlet/UserCustomerServlet?action=getUserDetail", parameters: dic, encoding: .URL, headers: nil)
		// 这块直接用的是这个方法 可以传入一个线程 适用于UI
		.responseJSON(queue: dispatch_get_main_queue(), options: []) { (response) in
			// 如果成功
			if response.result.isSuccess {
				...
				// 如果返回值中没有要取的字段
					...
					finished(author: nil, error: NSError(domain: "在返回的结果中没有找result字段", code: 44, userInfo: nil))
				}
			} else {
				// 失败将错误返回
